...
function computeStats(bookmarks: Bookmark[], history: StatusHistoryEntry[]): Stats {
  ...
  const completedThisWeek = completions.filter((h) => new Date(h.changed_at) >= weekAgo).length;

  const completedThisMonth = completions.filter((h) => new Date(h.changed_at) >= monthAgo).length;

  // Streak: consecutive days with at least one completion
  const completionDates = new Set(completions.map((h) => localDateString(new Date(h.changed_at))));
  let currentStreak = 0;
  ...
  if (completedBookmarks.length > 0) {
    ...
    avgCompletionDays =
      Math.round((totalMs / completedBookmarks.length / (1000 * 60 * 60 * 24)) * 10) / 10;
  }
