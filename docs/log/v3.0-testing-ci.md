...
## Known issue fixed (2026-02-18)

**CI was failing** because `src/lib/supabase.ts` throws `Error('Missing VITE_SUPABASE_URL...')` when env vars are absent. CI has no `.env.local`, so both `npm run test` and `npm run build` failed.

**Fix:** Added `env:` block at job level in `.github/workflows/ci.yml` to consume GitHub Actions secrets:
...
The anon key is a Supabase publishable key (intentionally public, restricted by RLS) — safe to store as a repo secret. Add under: GitHub repo → Settings → Secrets and variables → Actions → New repository secret.
