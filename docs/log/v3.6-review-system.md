...
## Bug Fix — ReviewCard crash on undefined areas

**Date:** 2026-02-21

`get_review_queue` returns raw bookmark rows with no join on `bookmark_tags`/`tag_areas`, so `bookmark.areas` is `undefined` at runtime. `ReviewCard.tsx` read `bookmark.areas.length` without a guard, causing a runtime crash for any review session.

**Fix:** `src/components/review/ReviewCard.tsx` lines 98 + 100 — `bookmark.areas` → `(bookmark.areas ?? [])` on both the length check and the `.map()`.

**SQL:** `sql/migrations/review-scheduler-v2.sql` run in Supabase — replaces the `get_review_queue` RPC to remove the hardcoded 7-day filter (client-side `isDue()` handles this now).
